...
def optimize_slope(support: bool, pivot: int, init_slope: float, y: np.array):
    ...
    best_err = check_trend_line(support, pivot, init_slope, y[pivot:])
    ...
    while curr_step > min_step:
        if get_derivative:
            ...
            test_err = check_trend_line(support, pivot, slope_change, y[pivot:])
        ...
        test_err = check_trend_line(support, pivot, test_slope, y[pivot:])
...
def test_sr_lines(data):
    import mplfinance as mpf

    candles = data.iloc[-50:]  # Last 30 candles in data
    ...
    mpf.plot(
        candles,
        alines=dict(alines=[s_seq, r_seq, s_seq2, r_seq2], colors=["w", "w", "b", "b"]),
        type="candle",
        style="charles",
        ax=ax,
    )
